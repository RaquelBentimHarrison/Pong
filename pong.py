"""
A simple take on the pong game. To move paddle use up and down keys
"""
...
BALL_CENTER_Y = random.randint(0, SCREEN_HEIGHT - BALL_RADIUS)
RANDOM_VELOCITY_BASE = -5
RANDOM_VELOCITY_MAX = 5

PADDLE_CENTER_X = SCREEN_WIDTH - 20
PADDLE_CENTER_Y = 25
PADDLE_WIDTH = 10
PADDLE_HEIGHT = 50
MOVE_AMOUNT = 5
...
class Point:
    """Class for a point"""
    def __init__(self):
        """Sets up Point Coordinates"""
        self.x = 0.0
        self.y = 0.0
        
class Velocity:
    """Class that defines the velocity of a Point"""
    def __init__(self):
...
class Ball:
    """This class sets up a Ball for the game"""
    def __init__(self):
        self.center = Point()
        self.center.x = BALL_CENTER_X
        self.center.y = BALL_CENTER_Y
        self.velocity = Velocity()
        self.velocity.dx = random.randint(RANDOM_VELOCITY_BASE, RANDOM_VELOCITY_MAX)
        self.velocity.dy = random.randint(RANDOM_VELOCITY_BASE, RANDOM_VELOCITY_MAX)
        
    def draw(self):
        arcade.draw_circle_filled(self.center.x, self.center.y, BALL_RADIUS, arcade.color.PINK)

    def advance(self):
        while self.velocity.dx == self.velocity.dy:
            self.velocity.dx = random.randint(RANDOM_VELOCITY_BASE, RANDOM_VELOCITY_MAX)
            self.velocity.dy = random.randint(RANDOM_VELOCITY_BASE, RANDOM_VELOCITY_MAX)
...
class Paddle:
    ...
    def __init__(self):
        self.center = Point()
        self.center.x = PADDLE_CENTER_X
        self.center.y = PADDLE_CENTER_Y
        
    def draw(self):
        arcade.draw_rectangle_filled(self.center.x, self.center.y, PADDLE_WIDTH, PADDLE_HEIGHT, arcade.color.BLACK)
    
    def move_up(self):
        if self.center.y < (SCREEN_HEIGHT - PADDLE_CENTER_Y):
            self.center.y += MOVE_AMOUNT
    
    def move_down(self):
        if self.center.y > (PADDLE_CENTER_Y):
            self.center.y -= MOVE_AMOUNT
